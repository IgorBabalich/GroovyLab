    ...
    break;
     ...
     if (e.isControlDown()) { // buffer the selected code
      ...
      break;
        ...
        /*Result lastResult = allResults.head();
        if (lastResult!=null)
       GlobalValues.consoleOutputWindow.append("result = "+ lastResult.value());
        }
        */
    /*
        if (GlobalValues.globalJavaREPLCLient==null)                
         GlobalValues.globalJavaREPLCLient = new JavaREPLClient(null, null);
     
      currentText = getSelectedTextOrCurrentLine();
      Option<EvaluationResult> result = null;
     
        {
            try {
                result =  GlobalValues.globalJavaREPLCLient.execute(currentText);
            } catch (Exception ex) {
                Logger.getLogger(EditorKeyMouseHandler.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        //   GlobalValues.GroovyShell.setVariable(x.key(), x.value());
        if (result.isDefined())
           GlobalValues.consoleOutputWindow.append(result.get().toString());
